defmodule Errorio.UserFromAuth do
  alias Errorio.User
  ...
  # Creates user and authorization entry for him
  def admin_insert_new_user(auth, repo) do
    register_user_from_auth(auth, nil, repo)
  end

  # updates user, then his authorization entry
  def admin_update_user(initial_user, user_params, auth_params, repo) do
    user_params = atomize_map(user_params)
    auth_params = atomize_map(auth_params)
    case repo.transaction(fn ->
      pw = Map.get(auth_params, :password)
      pwc = Map.get(auth_params, :password_confirmation)
      email = Map.get(user_params, :email) || initial_user.email
      case validate_update_password(pw, pwc, email) do
        :ok ->
          result_user = User.changeset(initial_user, user_params) |> repo.update
          case result_user do
            {:ok, user} ->
  ...
  defp create_user(auth, repo) do
    ...
    result = User.changeset(%User{}, scrub(%{email: auth.info.email, name: name}))
    |> repo.insert
    case result do
      {:ok, user} -> user
      {:error, _reason} -> repo.rollback(:email_already_exists)
  ...
  defp token_from_auth_params(auth_params, token) do
    case auth_params do
      %{ password: pass } when (not is_nil(pass) and not (byte_size(pass) == 0))  ->
        Comeonin.Bcrypt.hashpwsalt(pass)
      %{"password" => pass} when (not is_nil(pass) and not (byte_size(pass) == 0)) ->
        Comeonin.Bcrypt.hashpwsalt(pass)
      %{ password: pass } when byte_size(pass) == 0 ->
        token
      _ -> nil
    end
  end
  ...
  # converts map with string keys to map with atom keys
  defp atomize_map(params) do
    for {key, val} <- params, into: %{}, do: {key |> to_string |> String.to_atom, val}
  end
end
